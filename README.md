...
Choose the one variant that matches your environment.
...
```groovy
// javax variant (Java 8+)
implementation 'io.github.hclimkr:pxl-javax:1.0.0-RC1'
```

```groovy
// jakarta variant (Java 17+)
implementation 'io.github.hclimkr:pxl-jakarta:1.0.0-RC1'
```
...
public class Employee {
    ...
    @PxlRowIndex            // (optional) 1-based spreadsheet row number of the row. Type: byte/short/int/long + wrapper classes (Byte/Short/Integer/Long)
...
The source lives only in `pxl-javax`, and `pxl-jakarta` is generated by string substitution at build time. The build/test commands, source editing rules, and test authoring rules are documented in
