...
- For import, `.sheet(...)` cannot be chained consecutively — there are two ways to read multiple sheets.
    ...
    - One sheet at a time: call `.sheet(...)` once per sheet on the same builder instance and run each through its final (execute) step. The source is given at the execute step, so open a fresh stream per call (files are opened and closed internally each time).  
      In this form the CSV chain takes a single source, so handing more than one file — or more than one name or stream — to `.fromFiles(List<File>)` / `.fromStreams(List<String>, List<InputStream>)` throws `PxlArgumentException` (`builder.import.csv.singleSourceOnly`) at the final method rather than reading the first source and dropping the rest.
...
| `exportTrim`                                                                                                   | `false`    | Whether to trim the string on export                                                                            |
