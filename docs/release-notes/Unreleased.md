...
Additive API change: `@PxlSheet` gains two elements and `PxlImportSheetOption` two fields; no existing signature moved and no type was removed. Three things change for a caller — where `importCsvCharset` / `importCsvDelimiter` may be declared, the exception type raised when either is unusable, and the wording of the export-side codec parse messages.
