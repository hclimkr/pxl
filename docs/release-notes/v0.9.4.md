...
Diagnostic message release for **PXL** — an export that fails while parsing a string value now says so in its own words, and the library's internal message keys are grouped by binding direction so that a message can no longer sit under a name that hides which side it comes from.

No API change and no behavioral change: the same failures are still reported by the same exception types at the same points. The only user-visible difference is the wording of the export-side codec parse messages, listed in the first highlight.
