...
CSV release for **PXL**: objects can now be written to CSV as well as read from it, and the sheets of one CSV workbook can carry their own encoding, delimiter and byte order mark in either direction.

Pre-1.0 release with **two breaking changes**, both of which a compiler will point at. `PxlFileFormat` and `PxlExcelEngine` moved to `io.github.hclimkr.pxl.type` — update the import, nothing else about either type changed. And the six `Pxl{Import,Export}{Workbook,Sheet,Column}Option` classes became immutable, dropping 65 setters and 6 no-argument constructors; build them with their builder, as the API already intended. A patch number carries this because `0.x` allows it, not because the changes are small — see the [CHANGELOG](../../CHANGELOG.md), where every breaking item is marked.
