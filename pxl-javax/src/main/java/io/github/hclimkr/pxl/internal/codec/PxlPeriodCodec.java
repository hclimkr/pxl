...
/**
 * Codec for {@link Period} column values - writes {@link Period} into cells on export and parses
 * cells/strings into {@link Period} on import.
 *
 * <p>Export uses the column's export pattern (computed over {@code now .. now+period}) when set, otherwise
 * the ISO-8601 form. On import, NUMERIC and BOOLEAN cells are interpreted as a number of days, and strings
 * are parsed by the column's import pattern when set, otherwise by ISO-8601 {@link Period#parse}.
 */
