...
/**
 * Codec for primitive {@code float} column values — parses cells and strings into {@code float} on import and writes
 * {@code float} into cells on export. A numeric cell (a {@code double}) is narrowed to {@code float}; boolean cells map to
 * 1.0/0.0. Because {@code float} cannot be {@code null}, blank input parses to {@code 0.0f}. Both import and export reject
...
final class PxlPrimitiveFloatCodec {
    ...
    static float parsePrimitiveFloatValue(final String s,
        ...
        if (Objects.nonNull(importDecimalFormatter)) {
            try {
                ...
                throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_PARSE_INVALID, String.valueOf(stringValue), "float"), parseException);
            }
        } else {
            try {
                floatValue = Float.parseFloat(stringValue);
            } catch (NumberFormatException numberFormatException) {
                throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_PARSE_INVALID, String.valueOf(stringValue), "float"), numberFormatException);
            }
        }
    ...
    static String buildPrimitiveFloatCell(final Cell cell,
        ...
        if (object instanceof String) {
            ...
            throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_CONVERT_UNSUPPORTED, object.getClass().getSimpleName(), "float"));
        }
    ...
    private static String makePrimitiveFloatExportString(final Float floatValue,
        ...
        if (Objects.nonNull(exportDecimalFormatter)) {
            try {
                ...
                throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_PATTERN_APPLY_FAILED, String.valueOf(floatValue)), illegalArgumentException);
            }
