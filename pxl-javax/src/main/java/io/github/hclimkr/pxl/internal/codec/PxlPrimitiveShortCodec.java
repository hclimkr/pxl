...
/**
 * Codec for primitive {@code short} column values - writes {@code short} into cells on export and parses cells and
 * strings into {@code short} on import. Numeric input is range-checked against the {@code short} range (throwing on overflow)
...
final class PxlPrimitiveShortCodec {
        ...
        if (object instanceof String) {
            ...
            if (StringUtils.isBlank(stringValue)) {
                ...
                try {
                    ...
                    throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_EXPORT_PARSE_INVALID, String.valueOf(stringValue), "short"), numberFormatException);
                }
            }
        } else if (object instanceof Short) {
            shortValue = (Short) object;
        } else {
            throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_EXPORT_CONVERT_UNSUPPORTED, object.getClass().getSimpleName(), "short"));
        }
    ...
    private static String makePrimitiveShortExportString(final Short shortValue,
        ...
        if (Objects.nonNull(exportDecimalFormatter)) {
            try {
                ...
                throw new PxlCellCodecException(PxlI18nDiagnostic.get(PxlI18nDiagnosticKeys.CODEC_EXPORT_PATTERN_APPLY_FAILED, String.valueOf(shortValue)), illegalArgumentException);
            }
     ...
     * ({@code PxlNumberSupport.parseFullyAsNumber}) and the parsed number is range-checked against the {@code short} range and
