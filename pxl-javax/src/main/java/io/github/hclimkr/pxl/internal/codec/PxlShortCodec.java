...
/**
 * Codec for {@code Short} column values — parses cells and strings into {@code Short} on import and writes {@code Short}
 * into cells on export. Numeric input is range-checked against the {@code Short} range (throwing on overflow) and truncated
...
final class PxlShortCodec {
     ...
     * @return the parsed {@code Short}, or {@code null} for a blank cell
    ...
    /**
     * Parses a string into a {@code Short}. Trims first when {@code importTrim} is enabled and returns {@code null} for
     * blank input. When an import {@link DecimalFormat} is configured the parsed number is range-checked against the
     * {@code Short} range and truncated; otherwise {@link Short#parseShort(String)} is used.
     *
     * @param s          the source string
     * @param columnMeta resolved import metadata for the column
     * @return the parsed {@code Short}, or {@code null} for blank input
